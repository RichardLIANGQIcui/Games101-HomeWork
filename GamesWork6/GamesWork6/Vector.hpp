...
public:
	...
	Vector2f operator *(const float& r) const { return Vector2f(r * x, r * y); }
...
public:
	...
	static Vector3f Min(const Vector3f& p1, const Vector3f& p2)//求包围盒最小边界
	{
		return Vector3f(min(p1.x, p2.x), min(p1.y,p2.y), min(p1.z, p2.z));
	}

	static Vector3f Max(const Vector3f& p1, const Vector3f& p2)//求包围盒最大边界
	{
		return Vector3f(max(p1.x, p2.x), max(p1.y, p2.y), max(p1.z, p2.z));
	}

};

inline double Vector3f::operator[](int index) const {
	return (&x)[index];//这里有点不太明白
}
