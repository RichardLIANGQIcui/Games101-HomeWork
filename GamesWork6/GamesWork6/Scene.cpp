	...
	if (intersection.happened)
	{
		...
		switch (m->getType())
		{
			case REFLECTION_AND_REFRACTION:
			{
				...
				hitColor = kr * reflectionColor + (1 - kr) * refractionColor;
				break;
			}
			case REFLECTION:
			{
			...
			default:
			{
				...
				for (uint32_t i = 0;i < get_lights().size();++i)
				{
					...
					}else
					{
						...
						Vector3f reflectionDirection = normalize(reflect(-lightDir, N));
						specularColor += powf(max(0.f, dotProduct(reflectionDirection, ray.direction)),
