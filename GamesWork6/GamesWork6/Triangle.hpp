...
public:
    ...
    MeshTriangle(const string& filename)
    {
        objl::Loader loader;
        ...
        Vector3f min_vert = Vector3f(numeric_limits<float>::infinity(),
            numeric_limits<float>::infinity(), numeric_limits<float>::infinity());

        Vector3f max_vert = Vector3f(-numeric_limits<float>::infinity(),
            -numeric_limits<float>::infinity(), -numeric_limits<float>::infinity());

        for (int i = 0;i < mesh.Vertices.size();i += 3)
        {
            array<Vector3f, 3> face_vertices;
            for (int j = 0;j < 3;j++)
            {
                ...
                min_vert = Vector3f(min(min_vert.x, vert.x),
                    min(min_vert.y, vert.y), min(min_vert.z, vert.z));

                max_vert = Vector3f(max(max_vert.x, vert.x),
                    max(max_vert.y, vert.y), max(max_vert.z, vert.z));
            }
